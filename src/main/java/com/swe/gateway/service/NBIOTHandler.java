...
public class NBIOTHandler {
    ...
    public Mono<String> insertNBIOTHandler(JSONObject jsonObject, String deviceID){
        ...
        for (int j = 0; j < dataList.size(); j++) {
            JSONObject list_obj = (JSONObject) dataList.getJSONObject(j);
            if (list_obj != null) {
        ...
        Mono <NBIOT> str = Mono.just(nbiot);
        return str.flatMap(monosink->{
            nbiotRepository.insertNBIOT(monosink);
            return Mono.just(monosink.toString());
        });
    }
}
